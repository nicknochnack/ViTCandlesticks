...
model = ViT().to('cuda')
model.load_state_dict(torch.load('checkpoints/75_model.pt', weights_only=True))
model.eval()

data = ClfDataset('consolidated_data') 
dataloader = DataLoader(data, batch_size=64, shuffle=True, prefetch_factor=2, num_workers=2)
